...
std::vector<std::string> ConverterJSON::GetTextDocuments()
{
	...
	if (json_data["config"].empty()) throw std::runtime_error("Config file is empty\n");
	...
	auto DOCS_BASE_LOAD = [&](const size_t& doc_id)
	{
		...
		{
			if (word_counter++ > 1000) throw std::runtime_error("More then 1000 words text\n");
			if (word.length() > 100) throw std::runtime_error("More then 100 chars word eException\n");
			text += word + " ";
		}
...
std::vector<std::string> ConverterJSON::GetRequests()
{
	...
	if (json_data["requests"].empty()) throw std::runtime_error("Request file is empty\n");
	if (json_data["requests"].size() > 1000) throw std::runtime_error("More then 1000 words text\n");
	...
	for (std::string request : json_data["requests"])
	{
		...
		if (word_counter > 10) throw std::runtime_error("More then 100 chars word eException\n");
	}
